/**
 * Filter resume data by tags
 */
export function filterByTags(data, tags, mode = 'any') {
  if (!tags || tags.length === 0) return data;

  const matchTags = (itemTags) => {
  ...
  return {
    ...data,
    education: data.education?.filter(edu => matchTags(edu.tags)),
    research_experience: data.research_experience?.map(exp => ({
      ...exp,
      projects: exp.projects?.filter(proj => matchTags(proj.tags))
    })).filter(exp => exp.projects?.length > 0),
    skills: filterSkillsByTags(data.skills, matchTags),
    projects: data.projects?.filter(proj => matchTags(proj.tags)),
    publications: {
      journal_articles: data.publications?.journal_articles?.filter(p => matchTags(p.tags)),
      patents: data.publications?.patents?.filter(p => matchTags(p.tags))
    },
    awards: {
      grants: data.awards?.grants?.filter(a => matchTags(a.tags)),
      honors: data.awards?.honors?.filter(a => matchTags(a.tags))
    },
    volunteer: data.volunteer?.filter(v => matchTags(v.tags))
  };
}

function filterSkillsByTags(skills, matchTags) {
  ...
  for (const [category, items] of Object.entries(skills)) {
    if (Array.isArray(items)) {
      const filtered = items.filter(item =>
        typeof item === 'string' || !item.tags || matchTags(item.tags)
      );
...
export function hasContent(data) {
  ...
  return hasEducation || hasExperience || hasSkills || hasProjects || hasPubs || hasAwards;
}
